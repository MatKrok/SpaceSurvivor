...
public class WorldScrolling : MonoBehaviour
{
    ...
    [SerializeField] int filedOfVisionHeight = 3;
    [SerializeField] int filedOfVisionWidth = 3;
    ...
    private void Start()
    {
        playerTransform.position = CalculateTilePosition(playerTilePosition.x, playerTilePosition.y); // Dodajemy liniê kodu
        UpdateTilesOnScreen();
    }
    ...
    {
        for(int pov_x = -(filedOfVisionWidth/2); pov_x <= filedOfVisionWidth/2;pov_x++)
        {
            for (int pov_y = -(filedOfVisionHeight/2); pov_y <= filedOfVisionHeight/2; pov_y++)
            {
                ...
                //Tile has to be moved if it was visited to prevent spawning object on the same tile
